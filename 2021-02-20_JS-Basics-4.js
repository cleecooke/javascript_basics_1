...
function threePhone(phoneBook) {
    let match = "333-333-3333",
        name;

    for (name in phoneBook) {

        if (phoneBook[name] === match) {
            console.log(name);
        }
...
function kindChecker(json) {
    let items = json.items,
        iLen = items.length,
        match = "shopping#product",
        i = 0,
        matchedCount = 0;

        for (i; i < iLen; i += 1) {

            if (items[i].kind === match) {
                matchedCount += 1;
            }

        }

    return "total products where kind = shopping#products: " + matchedCount.toString();
}

// console.log(kindChecker(json));      //2.a "Print the count of these results",
                                            // the JSON.current item counts is where it's stored


// 2. b

function backOrderChecker(json) {
    let items = json.items,
        iLen = items.length,
        match = "backorder",
        i = 0,
        ...
        for (i; i < iLen; i += 1) {
            ...
            if (items[i].product.inventories[0].availability === match) {
                backOrderedItems.push(items[i].product.title);
            }

        }

    return backOrderedItems.join(", \n");
}
// console.log(backOrderChecker(json));


// 2. c

function imgCntChecker(json) {
    let items = json.items,
        iLen = items.length,
        i = 0,
        multiImgItems = [];
        ...
        for (i; i < iLen; i += 1) {
            ...
            if (items[i].product.images.length > 1) {
                multiImgItems.push(items[i].product.title);
            }

        }

    return multiImgItems.join(", \n");
}
// console.log(imgCntChecker(json));


// 2. d

function findCanon(json) {
    let items = json.items,
        iLen = items.length,
        match = "Canon",
        i = 0,
        ...
        for (i; i < iLen; i += 1) {
            ...
            if (items[i].product.title.includes(match)) {
                canonItems.push(items[i].product.title);
            }

        }

    return canonItems.join(", \n");
}
// console.log(findCanon(json));



// 2. e

function findEbayCanon(json) {
    let items = json.items,
        iLen = items.length,
        titleMatch = "Canon",
        authorMatch = "eBay",
        ...
        for (i; i < iLen; i += 1) {
            ...
            if (items[i].product.title.includes(titleMatch) && items[i].product.author.name.includes(authorMatch)) {
                canonItems.push(items[i].product.title);
            }
...
// console.log(findEbayCanon(json));
...
function productDisplay(json) {
    let items = json.items,
        ...
        for (i; i < iLen; i += 1) {
            ...
            console.log(
                items[i].product.brand + "\n" + 
                "$" + items[i].product.inventories[0].price + "\n" +
                items[i].product.images[0].link + "\n\n"
            )
...
console.log(productDisplay(json));